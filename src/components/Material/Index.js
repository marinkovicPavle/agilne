...
class Index extends Component
{
	...
	handleDelete(itemToDelete) {
        deleted = true;
        var referenceToDelete = firebase.database().ref('material').orderByChild('id').equalTo(itemToDelete.id);
        ...
        toastr.error('Material has been deleted successfully!', {position : 'top-right', heading: 'Done'});
	}
      ...
      return(
          	<div className="card mt-4">
			    <div className="card-header">
                    ...
                    {userLoggedIn &&
                        <button type="button" className="btn btn-primary btn-sm pull-right" data-toggle="modal" data-target="#addModal" onClick={this.handleDeleted()}> Add Material </button>
                    }
			    </div>
			        ...
			        <div className="col-md-12">
                        <table className="table table-bordered">
                            ...
                            {this.state.originalData.map((data, i) => (
                                <tr key={i}>
                                    <td> {data.authorName + ' ' + data.authorSurname} </td>
                                    <td> {data.publisher} </td>
                                    <td> {data.date} </td>
                                    <td> {data.publisherCity} </td>
                                    <td>
